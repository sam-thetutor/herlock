...
module {
    ...
    func build_transaction(
        ...
        let max_iterations : Nat = 20; // Prevent infinite loops
        loop {
            if (iterations >= max_iterations) {
                Debug.print("Transaction building reached max iterations, using current fee");
            ...
            let calculated_fee = (signed_tx_bytes_len * fee_per_vbyte_nat) / 1000;

            // Check for convergence: fee matches OR fee is within 1 satoshi (to handle rounding)
            if (calculated_fee == total_fee or (calculated_fee > total_fee and calculated_fee - total_fee <= 1) or (total_fee > calculated_fee and total_fee - calculated_fee <= 1)) {
                Debug.print("Transaction built with fee " # debug_show (total_fee) # " after " # debug_show (iterations) # " iterations");
                return transaction.toBytes();
            } else {
